...
class TwoLayerNet():
    ...
    def forward(self, X, y=None):
        ...
        h = np.dot(X,W1) + b1 # XW1 + b1
        a = h * (h > 0) # ReLU(h)
        scores = np.dot(a, W2) + b2 # AW2 + b2

        o = np.exp(scores)
        p = o / np.sum(o, axis = 1).reshape(-1,1) # Softmax(S)
    ...
    def backward(self, X, y, learning_rate=1e-5):
        ...
        grads["W2"] = np.dot(a.T, dp/len(y)) # dL/dW2 (H,C)
        grads["b2"] = np.sum(dp/len(y), axis = 0) # dL/db2 (N,C)

        da = np.dot(dp,W2.T) # (N,H)
        da[a<=0] = 0 # ReLU, boolean indexing
        dh = da # (N,H)

        grads["W1"] = np.dot(X.T, dh/len(y)) # (D,H)
        grads["b1"] = np.sum(dh/len(y), axis = 0) # (N,H)
